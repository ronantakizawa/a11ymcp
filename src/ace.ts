...
export interface AceApi {
  setConfig(config: AceConfig): Promise<void>;
  getCompliance(page: unknown, label: string): Promise<{ report: unknown }>;
  close(): Promise<void>;
}

export interface AcePage {
  url(): string;
  viewport(): { width: number; height: number } | null;
}

export interface AceScanOptions {
  tags?: string[];
  policies?: string[];
  label?: string;
}

interface AceSummaryCounts {
  ignored?: number;
  elements?: number;
...
interface AceReportInput {
  results?: AceIssueInput[];
  numExecuted?: number;
  summary?: {
    counts?: AceSummaryCounts;
    scanTime?: number;
    ruleArchive?: string;
    policies?: string[];
    reportLevels?: string[];
...
export interface AceNormalizationContext {
  url: string;
  viewport: { width: number; height: number } | null;
}

const defaultAceApi: AceApi = {
  setConfig: (config) => accessibilityChecker.setConfig(config as never),
  getCompliance: (page, label) => accessibilityChecker.getCompliance(page, label),
  close: () => accessibilityChecker.close(),
};

let aceQueue: Promise<void> = Promise.resolve();

function uniqueNonEmpty(values: string[]): string[] {
  return [...new Set(values.map((value) => value.trim()).filter(Boolean))];
}

export function deriveAcePolicies(tags: string[] = [], policies: string[] = []): string[] {
...
function normalizeIssue(issue: AceIssueInput) {
...
export function normalizeAceReport(report: unknown, context: AceNormalizationContext) {
  const aceReport = report as AceReportInput;
  ...
  return {
    ...
    needsReview: issues.filter((issue) =>
      ['potentialviolation', 'potentialrecommendation', 'manual'].includes(issue.level),
    ),
...
function enqueueAceScan<T>(work: () => Promise<T>): Promise<T> {
  const result = aceQueue.then(work, work);
  aceQueue = result.then(
    () => undefined,
    () => undefined,
  );
  return result;
...
export function runAceScan(
  page: AcePage,
  options: AceScanOptions = {},
  api: AceApi = defaultAceApi,
) {
  return enqueueAceScan(async () => {
    ...
    let checkerResult: { report: unknown };
    try {
      await api.setConfig({
        ruleArchive: 'latest',
        policies,
